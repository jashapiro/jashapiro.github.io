...
opts_chunk$set(fig.path="plots/abalone_cleaning-", tidy=FALSE, fig.width=6.5, fig.height=5, fig.align="center", fig.retina = 2)
set.seed(20130409)
```
...
abalone <- filter(abalone, Height < 80,  Height > 0)
```
...
abalone <- filter(abalone, Whole - Shucked >= Shell)
```
...
ggplot(abalone, aes(x = Length, y = Height, color = Sex)) +
  geom_point() 
```
