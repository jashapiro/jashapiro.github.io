...
Now let's see how well our estimate of the population size from a capture-recapture experiment actually works. Let's assume that the true population size is exactly 2000 lizards. On the first trip we capture and mark 50. When we rerelease them, then the total population will consist of 100 marked individuals and 1950 unmarked lizards, which we will represent as a vector of strings: 100 with the value of `"marked"` and the remaining 1950 `"unmarked"`. On our second trip we will capture 150 individuals, just as in the example above. We will simulate this capture using the `sample()` function, which picks elements from a list randomly without replacement (unless you tell it differently). We will sample 150 individuals from the "population" vector, then count the number of those that are `"marked"`.
...
{% highlight text %}
##  [1] 2500 1875 1500 3750 1071 3750 2500 1250 2500 2500
{% endhighlight %}
