...
opts_chunk$set(fig.path="plots/abalone_cleaning-", tidy=FALSE, fig.width=6.5, fig.height=5, cache=TRUE, cache.extra=rand_seed)
set.seed(20130409)
```
...
```{r trimdata, echo=FALSE, include=FALSE}
abalone <- subset(abalone, Height < 80 & Height > 0)
```
...
If you did everything as I did, you should now have an abalone data frame with `r nrow(abalone)` rows. Also, if you make a new version of the Whole vs. Shucked plot, you will should see that most of the dodgy points are now gone. 
...
## Saving your work
So now, finally, you should have an abalone data frame with `r nrow(abalone)` rows. Remember how I said that dealing with bad data was a big part of the job? Exactly.
